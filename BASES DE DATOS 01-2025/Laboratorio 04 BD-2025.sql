...
-- SOLUCION DE CADA UNO DE LOS EJERCICIOS 

--Ejercicio 1
SELECT E.nombre, P.nombre FROM Empleado E INNER JOIN Puesto P ON P.id = E.id_puesto

--Ejercicio 2
SELECT T.nombre, T.version FROM Tecnologia T

--Ejercicio 3
SELECT A.nombre AS 'Nombre Area', C.nombre 'Nombre Categoria' FROM Area A INNER JOIN Categoria C ON A.id = C.id_area

--Ejercicio 4
SELECT C.tema, C.fecha FROM Capacitacion C

--Ejercicio 5
SELECT P.titulo, C.nombre FROM Proyecto P INNER JOIN Cliente C ON P.id_cliente = C.id

--Ejercicio 6
SELECT P.titulo, C.nombre 'Nombre del cliente', E.nombre + ' ' + E.apellido 'Nombre del lider de proyecto' FROM Proyecto P INNER JOIN Cliente C ON P.id_cliente = C.id INNER JOIN Empleado E ON E.id = P.id_lider

--Ejercicio 7
SELECT * FROM Proyecto P INNER JOIN TecnologiaXProyecto TP ON P.id = TP.id_proyecto WHERE TP.id_tecnologia = 2

--Ejercicio 8
SELECT * FROM Area A INNER JOIN Categoria C ON C.id_area = A.id WHERE A.nombre = 'Tecnología'

--Ejercicio 9
SELECT * FROM Empleado E LEFT JOIN Evaluacion Ev ON E.id = Ev.id_empleado WHERE Ev.id_empleado IS NULL

--Ejercicio 10
SELECT P.titulo, A.nombre 'Area', A.precio , T.nombre 'Tecnologia', T.version FROM Proyecto P 
INNER JOIN Categoria C ON P.id_cliente = C.id 
INNER JOIN Area A ON C.id_area = A.id 
INNER JOIN TecnologiaXProyecto TP ON TP.id_proyecto = P.id
INNER JOIN Tecnologia T ON T.id = TP.id_tecnologia
WHERE A.nombre = 'Tecnología' AND T.nombre = 'Python'

--Ejercicio 11
SELECT E.nombre + ' ' + E.apellido 'Nombre completo', P.id, P.titulo, Pu.nombre 'Puesto', D.nombre 'Departamento'  FROM EMPLEADO E
RIGHT JOIN Proyecto P ON E.id = P.id_lider
INNER JOIN Puesto Pu ON Pu.id = E.id_puesto
INNER JOIN Departamento D ON D.id = E.id_departamento
WHERE D.nombre = 'Marketing' AND E.nombre LIKE '%u%'

--Ejercicio 12
SELECT E.nombre + ' ' + E.apellido 'Nombre completo', D.nombre 'Departamento', E.fecha_contratacion FROM Empleado E
LEFT JOIN Asistencia A ON E.id = A.id_empleado
LEFT JOIN Proyecto P ON P.id_lider = E.id
INNER JOIN Departamento D ON D.id = E.id_departamento
WHERE A.id_empleado IS NULL AND P.id_lider IS NULL AND E.id_departamento = 2 AND E.fecha_contratacion > '2020-01-01'