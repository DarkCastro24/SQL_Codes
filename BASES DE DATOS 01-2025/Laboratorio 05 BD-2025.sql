...
INSERT INTO TecnologiaXProyecto VALUES (4, 6);
-- EJECUTAR HASTA ACA

-- ESTO EJECUTENLO APARTE (EXCLUSIVO DEL LABORATORIO 5)

ALTER TABLE Proyecto ADD Precio float
ALTER TABLE Proyecto ADD Complex BIT DEFAULT 0 WITH VALUES
ALTER TABLE Empleado ADD Star BIT DEFAULT 0 WITH VALUES
...
-- EJERCICIOS PROPUESTOS (SOLO PARA INSTRUCTOR)

-- SET 1 – Subconsultas básicas y funciones agregadas (Parte 1)

--Ejecicio 1
--Se necesita el nombre y el promedio de puntaje en las evaluaciones de cada empleado. Tenga en cuenta lo siguiente:
--Solo nos interesan aquellos empleados que superan el promedio de puntajes totales de todas las evaluaciones
--Solo nos interesan empleados que tienen evaluaciones; se puede ignorar a aquellos que no han recibido evaluaciones
SELECT E.nombre, AVG(EV.puntaje) FROM Empleado E 
INNER JOIN Evaluacion EV ON E.id = EV.id_empleado
GROUP BY E.id, E.nombre
HAVING (AVG(EV.puntaje)) > (SELECT AVG(puntaje) FROM Evaluacion)

--Ejercicio 2
--Se necesita agregar un valor de verdadero a la columna de Complex a aquellos proyectos que cumplan cualquiera de las dos condiciones:
--Cuyo precio total (la suma del precio del proyecto mas la suma del precio del area) sea mayor al promedio de precio total de todos los proyectos
--Cuya area sea Metodolog�as de trabajo

UPDATE Proyecto SET Complex = 0

UPDATE Proyecto SET Complex = 1 WHERE id IN
(SELECT P.id FROM Proyecto P
INNER JOIN Categoria C ON C.id = P.id_categoria
INNER JOIN Area A ON A.id = C.id_area
WHERE P.Precio + A.precio > 
(SELECT AVG(P.Precio + A.precio) FROM Proyecto P
INNER JOIN Categoria C ON C.id = P.id_categoria
INNER JOIN Area A ON A.id = C.id_area)
) OR id IN
(SELECT P.id FROM Proyecto P
INNER JOIN Categoria C ON C.id = P.id_categoria
INNER JOIN Area A ON A.id = C.id_area
WHERE A.nombre = 'Metodolog�as de trabajo')

SELECT * FROM Proyecto

--Ejercicio 3: Se necesita el conteo de los empleados en cada departamento cuyo puntaje promedio sea mayor al promedio de puntajes general

SELECT D.nombre ,COUNT(*) 'Numero de empleados' FROM Departamento D
...
--Ejercicio 4: Se necesita el conteo de capacitaciones al que ha asistido cada empleado, sin embargo, solo nos interesan los 3 empleados que han asistido a mas capacitaciones

SELECT TOP 3 E.nombre + ' ' + E.apellido AS 'Nombre completo', COUNT(A.id_empleado) AS 'Total capacitaciones' FROM Empleado E
INNER JOIN Asistencia A ON A.id_empleado = E.id
GROUP BY E.nombre, E.apellido
ORDER BY COUNT(A.id_empleado) DESC

--Ejercicio 5: Se necesita el conteo de asistentes por capacitacion, sin embargo, solo nos interesan aquellas capacitaciones que superan el promedio de asistencias a todas las capacitaciones
--Este promedio se calcula primero contando cuantas asistencias hay por capacitacion y luego se calcula el promedio de estos datos.
SELECT C.tema, COUNT(A.id_capacitacion) 'Total Asistentes' FROM Capacitacion C 
LEFT JOIN Asistencia A ON C.id = A.id_capacitacion
GROUP BY C.tema
HAVING COUNT(A.id_capacitacion) >
(SELECT AVG(A.[Total asistencias]) FROM 
(SELECT A.id_capacitacion, COUNT(A.id_capacitacion) 'Total asistencias' FROM Asistencia A
GROUP BY A.id_capacitacion) A)

/*
SET 2
*/
--Ejercicio 1
--Se necesitan los proyectos cuyo precio total (la suma del precio del proyecto mas la suma del precio del area)  sea mayor al precio total maximo en el area financiera

SELECT P.titulo, (P.Precio + A.Precio) 'Precio total' FROM Proyecto P
INNER JOIN Categoria C ON C.id = P.id_categoria
INNER JOIN Area A ON A.id = C.id_area
WHERE (P.Precio + A.precio) >  
(SELECT MAX(P.Precio + A.precio) FROM Proyecto P
INNER JOIN Categoria C ON C.id = P.id_categoria
INNER JOIN Area A ON A.id = C.id_area
WHERE A.nombre = 'Financieros')

--Ejercicio 2
--Se necesita el total de empleados (contando el empleado lider) por proyecto, pero solo aquellos proyectos que tienen mas de 4 empleados (nuevamente, contando al lider)

SELECT  P.titulo, COUNT(E.id) + 1 'Total Empleados' FROM Empleado E
INNER JOIN Trabajadores T ON E.id = T.id_empleado
INNER JOIN Proyecto P ON P.id = T.id_proyecto
GROUP BY P.titulo
HAVING COUNT(E.id) + 1 >= 5

--Ejercicio 3
--En recompensa a los lideres que mas han recaudado, se les agregara el valor de verdadero a la columna Star de la tabla empleado lo cual lo hara merecedor de un aumento.
--Los empleados que seran asignados son aquelllos cuyo promedio en ganancias de proyectos (entiendase ganancia como el precio total del proyecto que lidero el empleado) sea mayor a 1200 dolares

UPDATE Empleado SET Star = 0

UPDATE Empleado SET Star = 1 WHERE Empleado.id IN (
SELECT E.id FROM Empleado E
INNER JOIN Proyecto P ON P.id_lider = E.id
INNER JOIN Categoria C ON C.id = P.id_categoria
INNER JOIN Area A ON A.id = C.id_area
GROUP BY E.id
HAVING AVG(A.precio + P.Precio) > 1200)

SELECT * FROM Empleado WHERE Star = 1

--Ejecicio 4: Se necesitan el nombre de los clientes y el promedio del precio total que se ha recaudado de dicho cliente. 
--Sin embargo, solo nos interesan aquellos cuyo promedio de precio total sea mayor al promedio general de precio total de proyectos.

SELECT CL.nombre, AVG(P.Precio + A.precio) 'Promedio recaudado por cliente' FROM Cliente Cl
INNER JOIN Proyecto P ON P.id_cliente = Cl.id
INNER JOIN Categoria C ON C.id = P.id_categoria
INNER JOIN Area A ON A.id = C.id_area
GROUP BY CL.nombre
HAVING AVG(P.Precio + A.precio) >
(SELECT AVG(P.Precio + A.precio) FROM Proyecto P 
INNER JOIN Categoria C ON C.id = P.id_categoria
INNER JOIN Area A ON A.id = C.id_area)

--Ejercicio 5: Se necesita el conteo y el promedio de puntaje de cada departamento, sin embargo, se debe cumplir por lo menos una de las siguiente condiciones:
--Que el conteo de empleados sea mayor que el conteo total de empleados dividido entre 4
--Que el promedio de puntajes de cada departamento sea mayor que el promedio de puntajes de todos los empleados

SELECT D.nombre, COUNT(E.id) 'Numero de empleados', AVG(EV.puntaje) 'Promedio de puntaje de empleados' FROM Departamento D
INNER JOIN Empleado E ON D.id = E.id_departamento
INNER JOIN Evaluacion EV ON EV.id_empleado = E.id
GROUP BY D.nombre
HAVING COUNT(E.id) > (SELECT COUNT(*)/4 FROM EMPLEADO E)
OR AVG(EV.puntaje) > (SELECT AVG(puntaje) FROM Evaluacion)
